...
% Last Modified by GUIDE v2.5 07-Mar-2013 15:31:06
...
ImShop{handles.gui_number}.pathin = pwd;
...
% handles    structure with handles and user data (see GUIDATA)
global ImShop
startfolder = ImShop{handles.gui_number}.pathin;
multiselect = 'on'; 
...
prompt = {'Folder (leave blank to select group)',...
          'Save Folder (leave blank to use source folder)',...
          'change channel order (comma separated list or leave blank)',...
          };
...
if isempty(opts{1})
    ...
    multiselect = 'on'; 
    [FileName,PathName,FilterIndex] = ...
        uigetfile({'*.lsm','LSM Files'},'Select LSM Images',startfolder,...
            'MultiSelect',multiselect);
elseif ischar(opts{1})
    allLSM = dir([opts{1},filesep,'*.lsm']);
    PathName = [opts{1},filesep];
    FileName = {allLSM.name};
    FilterIndex = length(allLSM)>1; 
else
    FilterIndex = 0; 
end    
...
if FilterIndex ~= 0 
    Nfiles = length(FileName);
    for i=1:Nfiles
        LSMextract([PathName,FileName{i}],'chns',eval(['[',opts{3},']'])  );
    end
    % save in ImShop data structure 
     ImShop{handles.gui_number}.pathin = PathName;
    set(handles.pathin,'String',PathName); 
