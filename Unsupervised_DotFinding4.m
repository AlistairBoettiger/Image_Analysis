...
% Levine Lab                                        Last Modified: 12/29/12
%
...
slidedate = '2013-01-10_no-dist\'; % '2013-01-06_no-dist\'; % '2011-04_and_earlier/';  %2011-06-20/';  %2011-05-22/'; %
...
%---- Dot Finding Parameters ----- %
   % dotfinder's parameters 
    ...
    FiltSize = 30;% 
    min_size = 30;% 
    min_int1  = 0.095;    % .075  5    ;% .05 % not necessary Fix at Zero
    min_peak1 = 13000;% 7500;% 5000;% 3000; %
...
disp('loading data...');
    ...
    for mRNAchn = 1:mRNA_channels % mRNAchn =2
        ...
        elseif mRNAchn ==2
            ...
            % resize;        
            DotLabels= DotLabels(minZ:Zs); 
            DotData = DotData(minZ:Zs);    
            Inds = Inds(minZ:Zs); 
            Ints = Ints(minZ:Zs); 
...
Rpars.minpeak = min_peak;     
  ...
  fxn_anlz_counting_data(folder,rawfolder,slidedate,stackfolder,fname,mRNA_channels,ver,'chns_flipped',chns_flipped);
