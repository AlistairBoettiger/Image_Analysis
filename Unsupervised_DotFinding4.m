...
folder = 'C:\Users\Alistair\My Documents\Projects\mRNA_counting\Data\2011-12\';%2011-04_and_earlier/'; %2011-06-20/'; %  2011-04_and_earlier/'; % % 2011-05-22/'; % 2011-06-20/'; %   '/Users/alistair/Documents/Berkeley/Levine_Lab/Projects/Enhancer_Modeling/Data/'; 
...
%---- Dot Finding Parameters ----- %
   % dotfinder's parameters 
    ...
    min_int  = 0.06;    %  5    ;% .05 % not necessary Fix at Zero
    FiltSize = 30;% 
    min_size = 30;% 
    min_peak = 6000;% 2000; %
...
%           figure(1); clf; imagesc(M);
   ...
   %     % imdata2 = regionprops(NucLabeled,'PixelIdxList','Area'); 
     ...
     %   C=NucLabel;
        ...
        for i=1:Nnucs; % i = 4
         ...
         %   C(NucLabel==nn) = mRNA_cnt(i);
            ...
            if showim == 1
                ...
                Raw_cnt(NucLabel==nn) = single(mRNA_cnt(i));
            end
        end
        ...
        %  % troubleshooting
%   figure(3); clf; imagesc(Plot_mRNA*mean(imdata.Area)); colorbar;
%   figure(6); clf; imagesc(Raw_cnt);  colorbar;
%   hold on;  plot(  dotC(:,1),dotC(:,2),'w+','MarkerSize',14 );
